...
p <- add_argument(p, "sparse", help="If the matrix is in sparse format")
p <- add_argument(p, "resolution", help="Seurat resolution")
p <- add_argument(p, "isNormalized", help="Are we working with counts or cell cycle/whatever scaled data. FALSE if counts.")
p <- add_argument(p, "variableFeatures", help="If isNormalized is TRUE we need the variable genes for Seurat")
