...
library(igraph)
library(patchwork)
cetux<-snakemake@input$clust[1]   #[['clust']][1]
nt_1<-snakemake@input$clust[2]
nt_2<-snakemake@input$clust[3]
output_plot<-snakemake@output[[1]]

dato_cet<- read.table(file = cetux,row.names = 1,sep=",",header = TRUE)
dato_nt_1<- read.table(file = nt_1,row.names = 1,sep=",",header = TRUE)
dato_nt_2<-read.table(file = nt_2,row.names = 1,sep=",",header = TRUE)

dato_cet$sample_name<- strsplit(cetux , split = "/")[[1]][10]
dato_nt_1$sample_name<-strsplit(nt_1 , split = "/")[[1]][10]
dato_nt_2$sample_name<-strsplit(nt_2 , split = "/")[[1]][10]

dato<-rbind(dato_cet,dato_nt_1,dato_nt_2)
...
ggplot(data=agg_df, aes(x = Group.1,y=percent,fill=Group.2)) +geom_bar(stat="identity")+geom_text(aes(label=percent), vjust=1.6,color="black", size=3.5)+ xlab("Sample") + ylab("%") +labs(fill='')+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
cc_nt_1<-snakemake@input$cc[2]
dato_cc_nt_1<- read.table(file = cc_nt_1,row.names = 1,sep=",",header = FALSE)
dato_cc_nt_1<-merge(dato_nt_1,dato_cc_nt_1,by='row.names')

cc_nt_2<-snakemake@input$cc[3]
dato_cc_nt_2<- read.table(file = cc_nt_2,row.names = 1,sep=",",header = FALSE)
dato_cc_nt_2<-merge(dato_nt_2,dato_cc_nt_2,by='row.names')


dato_cc<-rbind(dato_cc_nt_1,dato_cc_nt_2,dato_cc_cet)
...
ggplot(data=agg_data, aes(x = Group.3,y=percent,fill=Group.2)) +geom_bar(stat="identity")+
  ...
  facet_wrap(~Group.1) +xlab("Cell Cycle") + ylab("%")  +labs(fill='')+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) 
graphics.off()
