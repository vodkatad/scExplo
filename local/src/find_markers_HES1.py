...
score=score.rename(columns={'HES_pos':'HES_pos_score','HES_neg':'HES_neg_score'})
print('porcodio')
print(score.head())
res=pd.concat([a, score], axis=1)
print('suka')
print(res.head())
paneth=res.loc[:,['HES_pos','HES_pos_score']]

#non_paneth=res.loc[:,['nPaneth','nPaneth_score']]
paneth.to_csv(snakemake.output.marker_HES_pos,sep='\t')
