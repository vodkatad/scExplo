...
for (i in colnames(dato)){
  print(i)
  p<-ggplot(chords, aes(x=X_umap1, y=X_umap2,color=dato[[i]])) + 
  geom_point(size=0.8)+scale_colour_gradientn(colours = rainbow(10),limits=c(0,10),guide = guide_colourbar())+#+scale_color_viridis(limits=c(0,10),direction = -1)
