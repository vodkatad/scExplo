...
a <- ggplot(df_combined, aes(x = HES1)) +
  ...
  #scale_y_continuous(expand = c(0, 0), limits = c(x_min, x_max), breaks = breaks_x) +  # Specifica i tick manualmente
  ggtitle("HES1") +
  theme_minimal() +
...
# Se desideri aggiungere un istogramma separato


# Calcolo dei limiti dell'asse y per la densità (se ancora necessario)
ggp <- ggplot_build(a)
maxy <- ceiling(max(ggp$data[[1]]$y))
